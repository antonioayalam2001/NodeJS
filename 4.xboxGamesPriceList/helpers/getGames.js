...
async function listOfGames(url) {
    ...
    const nameArray = await page.evaluate(
        () =>
            [...document.querySelectorAll("h3.base.mb-2.h6 a")].map(
    ...
    const priceArray = await page.evaluate(
        () =>
            [...document.querySelectorAll("div.card-body p[aria-hidden=true] span")].map(
                (partner) => {
                    if (partner.innerText != '+')
                        return partner.innerText
                    if (partner.innerText != 'con') return partner.innerText
                    return 
                }
            )
    );
    ...
    console.log(nameArray, priceArray, imgArray);

    let price = priceArray;
    price = priceArray.filter((e) => e !== "+"); // Filters out unwanted items
